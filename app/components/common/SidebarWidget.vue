...
<template>
  <aside class="w-full space-y-6">
    ...
    <div class="bg-white p-5 rounded-sm border border-gray-200">
      <h3
        class="text-base font-semibold text-gray-900 pb-3 mb-4 border-b-2 border-[#206be7] flex items-center justify-between m-0"
      >
        <span>快捷导航</span>
      ...
      <ul class="space-y-1.5 list-none p-0 m-0">
        <li v-for="item in navItems" :key="item.path">
          <NuxtLink
            :to="item.path"
            class="flex items-center justify-between px-3.5 py-2.5 rounded-sm text-sm transition-all"
            :class="isActive(item.path)
              ? 'bg-[#206be7] text-white font-medium shadow-xs'
    ...
    <div class="bg-white p-5 rounded-sm border border-gray-200">
      ...
      <ul class="grid grid-cols-2 gap-3 list-none p-0 m-0">
        <li v-for="c in recentCases" :key="c.id" class="group">
          <div
            class="border border-gray-200 rounded-sm overflow-hidden hover:border-[#206be7] transition-all bg-white hover:-translate-y-0.5"
          >
            ...
            <h4 class="p-2 m-0 text-xs font-normal text-center truncate">
    ...
    <!-- 联系咨询 Widget -->
    <div class="bg-gradient-to-br from-blue-50/60 to-white p-5 rounded-sm border border-blue-100">
      <h3 class="text-base font-semibold text-gray-900 pb-3 mb-4 border-b-2 border-[#206be7] m-0">
        <span>服务咨询</span>
      </h3>
      <div class="space-y-3 text-xs text-gray-600">
        ...
        <NuxtLink
          to="/contact"
          class="inline-flex items-center justify-center gap-2 w-full text-center text-xs py-2 mt-2 rounded-sm bg-[#206be7] hover:bg-[#1162e8] text-white font-medium shadow-xs hover:shadow-md transition-all duration-200 cursor-pointer"
        >
          <span>立即提交需求</span>
