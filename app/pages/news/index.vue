      ...
      <div class="flex flex-col lg:flex-row gap-9 items-start mb-15">
        <main class="flex-1 min-w-0 w-full">
              ...
              <div class="bg-white border border-gray-200 rounded-sm overflow-hidden flex flex-col h-full transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:border-gray-300 group">
                ...
                <div class="p-4 flex flex-col flex-1">
                  ...
                  <div class="flex items-center justify-between text-xs text-gray-400 pt-2.5 border-t border-slate-100 mt-auto">
                    <NuxtLink :to="`/news?category=${news.category}`" class="text-[#206be7] bg-blue-50/80 px-2 py-0.5 rounded-xs font-medium hover:underline">
                      {{ news.categoryLabel }}
                    </NuxtLink>
                    <span>{{ news.date }}</span>
                  </div>
          ...
          <ul v-else class="space-y-0 divide-y divide-gray-200 list-none p-0 m-0 mb-10">
            <li v-for="news in paginatedNews" :key="news.id" class="flex flex-col sm:flex-row gap-6 py-6 first:pt-0 group">
              ...
              <div class="flex-1 min-w-0 flex flex-col justify-between">
                <div>
                  <h2 class="text-base md:text-lg font-semibold leading-snug m-0 mb-2.5">
                    <NuxtLink :to="`/news/${news.id}`" :title="news.title" class="text-gray-900 group-hover:text-[#206be7] transition-colors">
                      {{ news.title }}
                    </NuxtLink>
                  </h2>
                ...
                <div class="flex items-center gap-3 text-xs text-gray-400">
                  <NuxtLink :to="`/news?category=${news.category}`" class="text-[#206be7] font-medium hover:underline">
                    {{ news.categoryLabel }}
                  </NuxtLink>
